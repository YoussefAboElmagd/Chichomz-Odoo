...
class SaleOrder(models.Model):
    ...
    def import_shopify_orders(self, order_data_lines, instance):
        ...
        for order_data_line in order_data_lines:
            ...
            for line in lines:
                _logger.info('Vendooooooooooor' + str(line.get("vendor")))
    ...
    def shopify_create_order(self, instance, partner, shipping_address, invoice_address,
        ...
        gateway = order_response.get('payment_gateway_names') or "no_payment_gateway"
        _logger.info(order_response)
        _logger.info(order_response.get('payment_gateway_names'))
        _logger.info(order_response.get('gateway'))
    ...
    def shopify_create_sale_order_line(self, line, product, quantity, product_name, price,
        ...
        line_vals = self.prepare_vals_for_sale_order_line(product, product_name, price, quantity,vendor=line.get("vendor"))
    ...
    def prepare_vals_for_sale_order_line(self, product, product_name, price, quantity, vendor):
        """ This method is used to prepare a vals to create a sale order line.
            @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 19 October 2020 .
        """
        final_vend = self.env['res.partner'].search([('name', '=', vendor)], limit=1)
        if not final_vend:
            final_vend = self.env['res.partner'].create({'name': vendor,'x_type_client':'supplier'})
